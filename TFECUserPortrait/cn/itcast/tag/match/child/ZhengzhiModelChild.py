...
class ZhengzhiMixin(AbstractBaseModelOpt):
    ...
    def compute(self, esDF: DataFrame, fiveDF: DataFrame):
        ...
        @F.udf
        def genderToTagsId(politicalface):
            return fiveRuleDict[str(politicalface)]

        # 5.2 查询esDF的gender, 作为参数, 编写udf函数, 实现传入gender, 作为key, 从字典获得value
        newDF = esDF.select(esDF.id.alias('userId'), genderToTagsId(esDF.politicalface).alias('tagsId'))
        # newDF.show(truncate=False)
        # esDF.select(esDF.politicalface).show(truncate=False)
        return newDF


if __name__ == '__main__':
    # 创建对象
    gm =ZhengzhiMixin('ZhengzhiModelChildTask')

    # 执行父类整合方法, 完整 性别标签计算即可.
    gm.execute()

    # 4. 打印结果
    logging.warning('ZhengzhiModelChildTask 任务执行成功')