...
@dataclass
class AbstractBaseModelOpt(object):
    '''
       思路总览:
           0. 准备Spark开发环境
           1. 读取MySQL中的数据
    ...
    def execute(self):
        ...
        # 我们先假设 新标签数据, 就是最终的结果数据, 然后"尝试"和旧标签合并.
        resultDF = newDF

        # 解决第一次执行标签计算任务, 旧的标签不存在的问题. 方案2: try except方式
        # 该解决方式的副作用: 必须保证6, 7步骤是正确的, 否则每次都会把当前标签计算结果写入到结果引擎.
        try:
            # 尝试要执行的代码, 如果这里没报错, 说明 旧索引数据是存在的.
            # 6. 查询elasticsearch中的oldDF
            ...
            # 走到这里, 说明上边的代码报错, 即: 旧标签数据不存在.
            logging.warning('注意: 仅第一个标签(newDF)会执行, 合并新旧标签有问题')
            self.__writeResulttoES(resultDF, esMeta)  # 只把新标签数据当做结果数据, 写出.
